...
**Полный редизайн UI завершён** по плану [`design-roadmap.md`](./design-roadmap.md). Все 14 шагов выполнены, `typecheck && lint && test:run` зелёные (161/161 тест).

- ✅ Эстетика «судовой журнал» — пергамент, чернила, штампы.
- ✅ Атмосферный слой по фазам (morning/day/evening/night) с CSS-lerp переходами.
- ✅ Шрифты: Caveat / Yeseva One / Spectral / JetBrains Mono.
- ✅ Палитра карт заблокирована: припасы `#6B8EA8`, навигация `#5BB5A8`, друг `#6E4878`, враг `#8C2C3D`, инвентарь `#5E5E5E`/`#7BB0D6`, рама `#7C2424`.
- ✅ Адаптивный ряд банок (N игроков = N мест, removed-банки скрыты).
- ✅ Хедер с фазовым штампом, счётчиком чаек, кнопками управления.
- ✅ POV-панель слева (за кого + друг + враг) — только в multiplayer.
- ✅ Колоды припасов и навигации справа с info-badge.
- ✅ Inventory rail снизу с названиями карт и слотами `#7BB0D6`.
- ✅ ActionPanel гибрид (нижняя полоса + сворачиваемые блоки сложных форм).
- ✅ Lobby и WaitingRoom в журнальном стиле (штампы, рукописные имена).
- ✅ LogPane (collapsible), Help (модальный свиток), DisconnectModal (порванная телеграмма).
- ✅ ScoringScreen — финальная страница журнала со звёздным фоном.
- ✅ SVG-иконография: 25+ собственных inline-SVG (нос, корма, чайка, компас, оружие, припасы и пр.) — ни одного эмодзи в дизайне.
- ✅ Info-badge на каждой карте с tooltip из словаря.
- ✅ Бой: хореография `attacker y=-32`, `defender y=+32`, союзники следуют, RESOLVE возвращает на места.
- ✅ Атмосфера: летящие чайки, плавающие волны, ползущий туман, звёзды ночью.
- ✅ Media-резолвер `src/ui/media.ts` — fallback на SVG-силуэт если картинки нет.

Структура медиа для будущих изображений: `src/assets/media/{characters,supplies,navigation,backgrounds}/{id}.{webp|png|jpg}`.
