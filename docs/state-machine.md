...
Этот документ описывает иерархическую state machine игры. Реализуется через **XState** в `src/game/machine.ts`. Машина — высокоуровневый координатор: знает, в какой фазе игра, какие подфазы возможны, и какие переходы валидны.
